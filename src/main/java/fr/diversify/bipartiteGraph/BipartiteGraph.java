...
public class BipartiteGraph{
    ...
    */
    ...
    public void initDependencies() throws Exception {
        ...
        for(Application app : applications) {
            ...
            for(int i = 0; i < borne; i++) {
                Platform p = list.get(r.nextInt(list.size()));
                while (p.full()) {
                    list.remove(p);
                    if(list.isEmpty())
                        throw new Exception("Pas de plateforme pour l'application: "+app);

                p = list.get(r.nextInt(list.size()));
                }
    ...
    protected void initServiceFor(Application app, NumberGenerator nbService) {
        int max = 0;
